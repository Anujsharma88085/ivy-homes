...
export const ListingDetail = () => {
  ...
  const isSaved = listing ? favouriteIds.has(listing.listing_id || listing.id) : false;
  ...
  useEffect(() => {
    const loadDetail = async () => {
      ...
      try {
        ...
        // Attempt /v1/listings/{id}/similar; if 404, synthesize client-side
        try {
          ...
          if (item) {
            try {
              const queryRes = await api.get('/v1/listings', {
                params: {
              ...
              const matches = (queryRes.data.results || [])
                .filter(l => (l.listing_id !== item.listing_id) && (l.bedroom === item.bedroom))
                .slice(0, 4);
              setSimilar(matches);
            } catch {
              setSimilar([]);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 space-y-8">
      <div className="flex items-center justify-between">
        ...
        {/* Quick Save Pill on top nav strip */}
        {/* <button
        ...
        >
          ...
          <span>{isSaved ? 'Saved to Favourites' : 'Save Property'}</span>
        </button> */}
      </div>
